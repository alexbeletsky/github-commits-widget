...
var element=widget.element;var user=widget.options.user;var repo=widget.options.repo;var branch=widget.options.branch;var last=widget.options.last==undefined?0:widget.options.last;var limitMessage=widget.options.limitMessageTo==undefined?0:widget.options.limitMessageTo;element.append('<h5>Widget intitalization, please wait...</h5>');gh.commit.forBranch(user,repo,branch,function(data){var commits=data.commits;var totalCommits=(last<commits.length?last:commits.length);element.empty();element.append('<ul>');for(var c=0;c<totalCommits;c++){element.append('<li>'+' '+avatar(commits[c].author.email)+' '+author(commits[c].author.login)+' commited '+message(commits[c].message,commits[c].url)+' '+when(commits[c].committed_date)+'</li>');}
element.append('</ul>');function avatar(email){var emailHash=hex_md5(email);return'<img src="http://www.gravatar.com/avatar/'+emailHash+'?s=20"/>';}
...
function when(commitDate){var commitTime=new Date(commitDate).getTime();var todayTime=new Date().getTime();var differenceInDays=Math.ceil(((todayTime-commitTime)/(24*3600*1000)));if(differenceInDays==0){return'just now';}
var r=''+differenceInDays+' days ago';return r;}});}
